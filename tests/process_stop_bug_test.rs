...
fn test_process_actually_killed_on_stop() {
    ...
    let pid = test_env.extract_pid_from_list("long_task")
    ...
    assert!(is_process_running(pid), "Process should be running before stop");
...
fn test_process_stop_without_handle() {
    ...
    assert!(is_process_running(pid), "Process should still be running after daemon restart");
