...
mod tests {
    ...
    async fn test_database_indexes_exist() {
        ...
        let result = sqlx::query("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_processes_status'")
            .fetch_one(&db.pool)
            .await;
        assert!(result.is_ok());
    }
...
use sqlx::{SqlitePool, Pool, Sqlite, migrate::MigrateDatabase};
